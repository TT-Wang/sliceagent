...
- *Authorization* (`policy.py`): an ordered `PolicyChain` behind the `PermissionHook`. Modes via `AGENT_POLICY`: `guard` (default — blocks catastrophic commands like `rm -rf /`, `sudo`, `curl … | sh`, writes to `/etc`, key/cred reads, force-push; normal dev commands pass), `readonly` (no writes/exec), `ask` (blocks catastrophic + interactively confirms every write/exec; "always" memorizes for the session), `allow` (permissive). Hooks can also mutate via `prepare_messages` (inject context before the LLM call) and `transform_tool_result` (rewrite/redact output before it enters the slice).

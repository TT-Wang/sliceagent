...
Ported from Hermes ``tools/fuzzy_match.py`` — ONLY the two whitespace-tolerant
strategies that are safe enough to anchor an edit on (line-trim + indentation-
flexible), plus their two position helpers. Stripped of all replacement logic:
this module NEVER writes. It only answers "where, if anywhere uniquely, does
``old`` live in ``content``?" as a single ``(start, end)`` character span.
...
Uniqueness gate (preserved from Hermes): a strategy that finds 0 or >1
candidates yields nothing. We try line-trim first, then indentation-flexible,
and return the span only when a strategy produces exactly ONE candidate.
"""
...
def fuzzy_find_unique(content: str, old: str) -> Tuple[int, int] | None:
    ...
    for strategy in (_strategy_line_trimmed, _strategy_indentation_flexible):
        ...
        if not matches:
            continue
        if len(matches) > 1:
            # Ambiguous under this strategy. Hermes treats >1 as a hard failure
            # (needs more context) rather than falling through to a looser
            # strategy that would only be more ambiguous. Mirror that.
            return None
        return matches[0]
...
# =============================================================================
# Matching strategies (ported from Hermes fuzzy_match.py:356 / :397)
# =============================================================================

def _strategy_line_trimmed(content: str, pattern: str) -> List[Tuple[int, int]]:
    """Match line-by-line after stripping leading/trailing whitespace per line.

    Ported from Hermes ``_strategy_line_trimmed`` (fuzzy_match.py:356).
    """
...
def _strategy_indentation_flexible(content: str, pattern: str) -> List[Tuple[int, int]]:
    ...
    Ported from Hermes ``_strategy_indentation_flexible`` (fuzzy_match.py:397).
...
# =============================================================================
# Position helpers (ported from Hermes fuzzy_match.py:650 / :669)
# =============================================================================
...
def _calculate_line_positions(content_lines: List[str], start_line: int,
    ...
    Ported verbatim from Hermes ``_calculate_line_positions`` (fuzzy_match.py:650).
...
def _find_normalized_matches(content: str, content_lines: List[str],
    ...
    Ported from Hermes ``_find_normalized_matches`` (fuzzy_match.py:669). Slides a
    window of ``len(pattern_norm_lines)`` lines across the normalized content; on
    each equal block it maps the line window back to an ORIGINAL-content char span
    via :func:`_calculate_line_positions`. Returns all such spans (the caller
    enforces uniqueness).
    """

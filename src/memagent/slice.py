...
SYSTEM_PROMPT = (
    ...
    "commands, edit code, configure or recover the environment). Respond to each message in kind: if it is a "
    "greeting, a question, or a request to explain, "
    ...
    "do NOT guess. Asking the user a follow-up is a normal, expected move, not a failure.\n"
    ...
    "success) before claiming it is done.\n"
    ...
    "When a single command could spew a LARGE dump (a disassembly, a long log, a large dataset), "
    ...
    "dropped field, a non-constant-time compare), so do not conclude too early and do not give up too early. "
    "(Minimize the SCOPE of your change, not the RIGOR of your verification — <work>'s 'act once you have "
    "enough' governs how much to BUILD, not how hard to CHECK.) Before "
    ...
    "</notes>\n\n"
    "<stop>\n"
    "When the change is complete and verified (per <verification>), write your final summary and "
...
MEMORY_ACCUMULATE = (
    ...
    "- OLDER turns of this session are PAGED OUT (not in the slice). The PAGED-OUT HISTORY section lists "
    "each (turn · title · note) WITH the exact recall_history call to bring it back. Reach back for what is "
    "NOT shown — rather than re-reading a file or re-deriving something you worked out earlier — but don't "
    "re-fetch what's already in front of you (RECENT / YOUR NOTES / OPEN FILES); recall_history(search=…) "
    "reaches OTHER sessions. Paging an earlier turn back is normal navigation, not a failure.\n"
